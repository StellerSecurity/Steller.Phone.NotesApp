...
export class NotesService {
  ...
  public setFailedPasswordNoteAttempts(noteId: string, attempts: number) {
    this.notesStorageService.setValue(
      this.getNoteFailedAttemptsKey(noteId),
  ...
  public markPendingMutation(
    ...
    this.pendingNoteMutations.set(noteId, {
      noteId,
      type,
      localUpdatedAt,
    });
  ...
  public hasPendingDelete(noteId: string): boolean {
    return this.pendingNoteMutations.get(noteId)?.type === 'delete';
  }

  public hasAnyPendingMutation(noteId: string): boolean {
    return this.pendingNoteMutations.has(noteId);
  }
